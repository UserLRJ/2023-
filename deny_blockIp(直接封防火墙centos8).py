    ...
    while True:
        ...
        if line:         
            ...
            if group and not deniesDict.get(group[1]):
                subprocess.getoutput('echo DenyUsers root@{} >> {}'.format(group[1],hostDeny))
                #外加防火墙直接封ip
                subprocess.getoutput("firewall-cmd --permanent --add-rich-rule='rule family='ipv4' source address='{}' reject'".format(group[1]))
                subprocess.getoutput("firewall-cmd --reload")
                deniesDict[group[1]] = '1'
                time_str = time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))
                print('{} --- add ip:{} to sshd_config for Invalid user'.format(time_str,group[1]))
                continue
            #用户名合法（有这个用户）密码错误
            ...
            if group:
                ...
                if tempIp[ip] > password_wrong_num and not deniesDict.get(ip):
                    ...
                    subprocess.getoutput('echo DenyUsers root@{} >> {}'.format(ip,hostDeny))
                    subprocess.getoutput("firewall-cmd --permanent --add-rich-rule='rule family='ipv4' source address='{}' reject'".format(group[1]))
                    subprocess.getoutput("firewall-cmd --reload")
                    deniesDict[ip] = '1'
                    time_str = time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))
                    print('{} --- add ip:{} to sshd_config for Failed password for root'.format(time_str,ip))
